...
pub(super) unsafe fn synthetic_throw(kind: SyntheticThrow) {
  ...
  unsafe { avanalyze_0_7_test_throw(kind as i32) }
}
...
#[link(name = "avanalyze_0_7_objc_cxx_barrier_test", kind = "static")]
unsafe extern "C-unwind" {
  /// `src/objc_cxx_barrier_test.mm` — four throws and a return.
  fn avanalyze_0_7_test_throw(kind: i32);
}
...
unsafe extern "C" {
  ...
  fn avanalyze_0_7_test_autorelease_sentinel();
  /// How many sentinels this process has deallocated.
  fn avanalyze_0_7_test_sentinel_deallocations() -> i32;
}
...
fn the_pool_pops_when_a_panic_passes_through_uncaught() {
  ...
  let before = unsafe { avanalyze_0_7_test_sentinel_deallocations() };

  let outcome = catch_unwind(AssertUnwindSafe(|| {
    guard_native("test_site", || {
      // SAFETY: allocates and autoreleases into the barrier's pool;
      // cannot raise.
      unsafe { avanalyze_0_7_test_autorelease_sentinel() };
  ...
  // SAFETY: as above.
  let after = unsafe { avanalyze_0_7_test_sentinel_deallocations() };
  assert_eq!(
    after,
    before + 1,
